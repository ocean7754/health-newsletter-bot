...
말투와 감정 표현:
- 실제 건강 카페 회원이 빠르게 작성한 것처럼 자연스러운 구어체를 사용한다.
- ㅠㅠ, ㅜㅜ, ㅋㅋ, ㅎㅎ, ^^, !!, ..., 아..., 하..., 헉 등의 표현을 상황에 맞게 섞는다.
- 10개 중 7~8개 글에는 감정 표현을 최소 1개 이상 포함한다.
- 감정이 강한 일부 글에는 감정 표현을 2~3개까지 사용할 수 있다.
- 모든 글에 같은 표현을 사용하지 않는다.
- 예를 들어 앞 글에서 ㅠㅠ를 사용했다면 다음 글에서는 ㅎㅎ, ㅋㅋ, !!, ^^ 또는 표현 없는 말투를 사용할 수 있다.
- ㅠㅠ와 ㅜㅜ는 속상함, 걱정, 답답함이 느껴지는 상황에서만 사용한다.
- ㅋㅋ와 ㅎㅎ는 민망함, 가벼운 자조, 예상 밖의 상황이나 소소한 웃음이 있는 글에 사용한다.
- ^^는 감사하거나 조심스럽게 의견을 묻는 글에 가끔만 사용한다.
- !!는 놀람, 결심, 강조가 자연스러운 부분에만 사용한다.
- 한 문장 끝에 감정 표현을 여러 개 몰아서 붙이지 않는다.
- "ㅠㅠㅎㅎ", "ㅋㅋㅠㅠ", "!!!!", "ㅎㅎㅎㅎㅎㅎ"처럼 과도하거나 어색하게 조합하지 않는다.
- 감정 표현을 문장마다 넣지 말고 글 전체에서 자연스럽게 분산한다.
- 문장 시작에도 "아...", "근데", "진짜", "하...", "와", "괜히", "은근" 같은 표현을 자연스럽게 사용할 수 있다.
- 일부 문장은 짧게 끊고, 일부 문장은 생각나는 대로 이어 쓰는 느낌으로 작성한다.
- 글마다 말투, 문장 길이, 문단 수, 시작과 마무리를 다르게 한다.
- 마지막을 모두 같은 질문 문장으로 끝내지 않는다.
사람마다 말투 차이를 만든다:
- 어떤 글은 차분하고 조심스럽게 쓴다.
- 어떤 글은 답답한 마음을 솔직하게 털어놓는다.
- 어떤 글은 자신의 실수를 가볍게 웃으며 이야기한다.
- 어떤 글은 걱정이 많아 생각이 계속 이어지는 말투로 쓴다.
- 어떤 글은 짧고 툭툭 끊기는 문장을 사용한다.
- 어떤 글은 비교적 긴 문장으로 상황을 자세히 설명한다.
- 모든 글에서 "~더라고요", "~인 것 같아요", "혹시 비슷한 분 계신가요?"를 반복하지 않는다.
- 같은 어미가 연속으로 반복되지 않도록 "~했어요", "~하네요", "~거든요", "~인가 봐요", "~싶어서요", "~네요" 등을 섞는다.
...
- 10개 중 7~8개 글에 ㅠㅠ, ㅜㅜ, ㅋㅋ, ㅎㅎ, ^^, !!, ... 중 상황에 맞는 표현을 넣는다.
- 같은 감정 표현이 연속된 게시글에 반복되지 않도록 분산한다.
- 감정 표현이 전혀 없는 차분한 글도 2~3개 포함한다.
